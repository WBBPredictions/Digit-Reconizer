...
Becker_Barton_Device <- function(Train, Test, mtry = 10, numtree = 100, gamma = 1/ncol(Train), cost = .01, svm.kernel = 'radial', K = 5)
{
  run_time <- list("SVM" = {}, "RF" = {}, "KNN" = {}, "Final_Selection" = {})
  
  s <- Sys.time()
  fit.svm <- svm(label ~ ., Train, type = "C-classification", cost = cost)
  pred.svm <- predict(fit.svm, Test)
  run_time$SVM <- Sys.time() - s
  s <- Sys.time()
  

  fit.rf <- randomForest(factor(label) ~ ., Train, mtry = mtry, ntree = numtree)
  pred.rf <- predict(fit.rf, Test)
  run_time$RF = Sys.time() - s
  pred.rf.prob <- predict(fit.rf, Test, type = 'prob')
  
  s <- Sys.time()
  pred.knn <- knn(train = Train[,-1], test = Test, cl = Train[,1], k = K)
  run_time$KNN <- Sys.time() - s
  
  pred.knn.prob <- knn(train = Train[,-1], test = Test, cl = Train[,1], k = K, prob = T)
  ...
  for(i in 1:length(pred.knn.prob))
  {
    ...
    {
      run_time$Final_Selection[i] = pred.knn[i]
    }
    else if(pred.rf.prob[i] > .6)
    {
      run_time$Final_Selection[i] = pred.rf
    }
    else
    {
      run_time$Final_Selection[i] = pred.svm[i]
    }
  }
...
s <- Sys.time()
pred.BBD.PCA <- Becker_Barton_Device(Train.pca, Test.pca)$Final_Selection
s <- Sys.time() - s
...
#BBD
temp <- data.frame("ImageId" = seq(1, 28000, 1), "Label" = pred.BBD.PCA)
temp$Label <- as.integer(temp$Label)
temp$ImageId <- as.integer(temp$ImageId)
...
#BBD
temp <- data.frame("ImageId" = seq(1, 28000, 1), "Label" = pred.BBD)
temp$Label <- as.integer(temp$Label)
temp$ImageId <- as.integer(temp$ImageId)
